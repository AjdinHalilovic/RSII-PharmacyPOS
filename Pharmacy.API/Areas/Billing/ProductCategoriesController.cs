...
namespace Pharmacy.API.Areas.Billing
{
    ...
    {
        public ProductCategoriesController(IDataUnitOfWork dataUnitOfWork) : base(dataUnitOfWork)
        {
        }
        ...
        public async Task<IActionResult> Get([FromQuery] CategorySearchObject search)
        {
            try
            {
                var roles = await DataUnitOfWork.BaseUow.ProductCategoriesRepository.GetByParametersAsync(search);

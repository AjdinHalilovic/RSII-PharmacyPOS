...
namespace Pharmacy.API.Areas.Billing
{
    ...
    {
        public InventoriesController(IDataUnitOfWork dataUnitOfWork) : base(dataUnitOfWork)
        {
        }
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                var inventories = await DataUnitOfWork.BaseUow.InventoriesRepository.GetAllDtosByPharmacyIdAsync(ClaimUser.PharmacyId, ClaimUser.InventoryId);
        ...
        {
            return Ok(await DataUnitOfWork.BaseUow.InventoriesRepository.GetByIdAsync(id));
        }
        #endregion

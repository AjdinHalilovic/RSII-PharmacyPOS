...
namespace Pharmacy.API.Areas.Settings
{
    ...
    {
        public AttributeOptionsController(IDataUnitOfWork dataUnitOfWork) : base(dataUnitOfWork)
        {
        }
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] AttributeOptionSearchObject search)
        {
            try
            {
                var attributeOptions = await DataUnitOfWork.BaseUow.AttributeOptionsRepository.GetAllByParametersAsync(search);
        ...
        {
            return Ok(await DataUnitOfWork.BaseUow.AttributeOptionsRepository.GetByIdAsync(id));
        }
        #endregion
